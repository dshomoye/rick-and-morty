pub mod Entity {
  ...
  impl API {
    ...
    pub async fn get<T>(&self, id: i64) -> Result<T, Error> where T: DeserializeOwned {
    ...
    pub async fn get_all<T>(&self) -> Result<Vec<T>, Error> where T: DeserializeOwned {
    ...
    pub async fn get_page<T>(&self, page: i64) -> Result<MultiPageResponse<T>, Error> where T: DeserializeOwned {
      let url = self.base_url() + "/?page=" + &page.to_string();
      let resp = reqwest::get(&url).await?.json::<MultiPageResponse<T>>().await?;
      Ok(resp)
    }

    /// Get multiple entities in vector of provided ids
    pub async fn get_multiple<T>(&self, pages: Vec<i64>) -> Result<MultiPageResponse<T>, Error> where T: DeserializeOwned {
    ...
    async fn get_url<T>(&self, url: &str) -> Result<T, Error> where T: DeserializeOwned {
      let resp = reqwest::get(url).await?.json::<T>().await?;
      Ok(resp)
    }
  }
